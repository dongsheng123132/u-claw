...
3. **不绑定设备、不打指纹、不上传数据** —— 开源版 2026-06-17 已移除这些逻辑，v3 不得回退。
   注意这句话的边界：它管的是**指纹与上传**，不是「宿主机零文件」。v3 默认会在本机建缓存，
   见下面的对外口径。这两件事不冲突，但**必须对客户明说**，不能让人从这句推出「什么都不留」。
4. **钱包凭证只在 U 盘** —— 见 §5。

### 对外口径与 portable-strict 模式（2026-08-23 定，用户拍板）

干净机实测：v3 默认模式会在宿主机留 **850.8 MB**（Node + 内核 + 缓存）。对此的口径**不是回避，是明说**：

**默认（快速模式，开箱即用的那个）**
- 运行时与缓存装到本机 `%LOCALAPPDATA%\U-Claw\`，业务数据（配置/会话/钱包）仍只在 U 盘
- **产品里必须明说**：首启会在这台电脑上建约 850 MB 缓存，占多少、在哪、怎么删，写在首启进度页和 README，不藏
- **必须给一键清理**：`runtime.host.purge` 动作 + 菜单入口。没有清理入口就不许发——
  「留东西」本身可以接受，「留了还删不掉」不可接受

**开关：portable-strict（全部装 U 盘）**
- 全部跑 U 盘、宿主机零残留。慢，但借用他人电脑、网吧、企业管控机需要
- 默认关闭，在配置页和启动器菜单里可切
- `%LOCALAPPDATA%` 不可写时（组策略、漫游配置文件）**自动降级到这个模式**，不许启动失败

> 决策记录：曾考虑「默认 strict 保住零痕迹观感」，未采纳 —— 那等于让 100% 的客户为
> 网吧/借用这类少数场景付 U 盘 IO 的代价，而这正是 v2 的病根（#39/#27/#43/#29/#37）。
> 选「默认快 + 明说 + 可清理 + 可切换」。
...
**v3 开发期间 main 不能静默。** 2026-08-23 已发 v2.1.18 并清完积压 issue（#24/#46/#48/#52/#53/#58 全部回复关闭，open 从 7 降到 3）。

> ⚠️ **上一版这里的结论是错的，留着当教训**：曾写「#48/#46 已于 2026-08-22 关闭——查证 `270cf4b` 早在 v2.1.10 就修了」。
> 那次"查证"只读了代码没上真机。真因是**内置 Node v22.22.1 跑不动 openclaw 2026.7.1 要求的 >=22.22.3**，
> 网关秒退 code 1，v2.1.15/16/17 三个下载包全是开不了机的。见 `918e3ca`。
> 教训（宪法第 4、5 条）：**「从代码上推断已修」不算验证**，可达性一律在真机上验。
...
| #48 / #46 `127.0.0.1:18789` 拒绝连接 | **CLOSED v2.1.18** | 真因是内置 Node 版本（`918e3ca`），不是启动时序；v3 里由通道钉版本 + `runtime.probe` 在装之前就拦住 |
| #53 运行方式只能单一 web | **CLOSED v2.1.18** | 菜单补了 [16] CLI 入口（`5d16081`）；真·多实例仍归 Phase 3 多 slot |
| #24 设计的启动不合理 | **CLOSED v2.1.18** | 强弹配置页已修（`b0c46d7`）；模型管理部分转 #61，首启流程仍归 Phase 0.5 |
| #58 微信 clawbot 扫码后不 work | **CLOSED v2.1.18** | 三个原因：配置整体覆盖冲掉 plugins（`b9c6f44`）+ 装错目录（`282b796`）+ 缺 zod（`1682b2d`） |
| #52 为什么不更新下载包了 | **CLOSED v2.1.18** | 包一直在发但最近三个是坏的（Node 版本）；另修了 latest.json 三条死链（`bcd1c5e`） |
