...
| `runtime-paths.mjs` | **v3 状态边界**：① 随盘走 `<usb>/data/`（配置/会话/钱包）② 本机共享 `<host>/shared/`（Node/内核/npm 缓存，跨 U 盘复用）③ 本机按盘隔离 `<host>/<slot16>/`（浏览器/编译缓存/日志/锁）。纯计算无副作用；建目录用 `prepareHostDirs()`。**`ok:false` 分两层处理，别混**：安装类动作（`runtime.seed/install`）**就该硬报错**（现状：`actions/runtime.mjs:208,267` 抛 `HOST_NOT_WRITABLE`，正确）；**启动器**才需要降级回落 U 盘直跑——用 `prepareRuntimePaths()`，它试本机、失败自动改挂 `<usb>/host/`（布局不变只换根），返回 `mode:'host'\|'portable-strict'\|'failed'`；只有 U 盘也写不了才 `ok:false`。**库已实现并有测试**（`tests/runtime-paths-fallback.test.mjs`，7 例）；⚠️ **但启动器还没调它** —— `Windows-Start.bat`/`Mac-Start.command` 仍是 v2 流程，没接 v3 安装链，所以今天不会引爆。引导器接入时必须走这个入口，别自己写 if。 |
