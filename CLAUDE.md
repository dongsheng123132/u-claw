...
| `runtime-paths.mjs` | **v3 状态边界**：① 随盘走 `<usb>/data/`（配置/会话/钱包）② 本机共享 `<host>/shared/`（Node/内核/npm 缓存，跨 U 盘复用）③ 本机按盘隔离 `<host>/<slot16>/`（浏览器/编译缓存/日志/锁）。纯计算无副作用；建目录用 `prepareHostDirs()`。**`ok:false` 分两层处理，别混**：安装类动作（`runtime.seed/install`）**就该硬报错**（现状：`actions/runtime.mjs:208,267` 抛 `HOST_NOT_WRITABLE`，正确）；**启动器**才需要降级回落 U 盘直跑——⚠️ **这一层尚未实现**，见 `docs/交接-2026-08-23-引导器与首启.md` 待办 3。目前生产启动路径还没接 v3 安装链，所以不会引爆；引导器接入前必须补上，否则受限账户（deny ACL）会从"U 盘直跑"变成启动失败。 |
